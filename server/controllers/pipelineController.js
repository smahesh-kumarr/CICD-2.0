...
export const getRecentActivities = async (req, res) => {
  try {
    ...
    const activities = await PipelineActivity.find(query)
      .populate({
        path: 'pipelineId',
        select: 'name type github docker aws sonarqube kubernetes testingTools continuousDeployment',
        populate: [
          { path: 'github.credentialId', select: 'name' },
          { path: 'docker.credentialId', select: 'name' },
          { path: 'aws.credentialId', select: 'name' },
          { path: 'sonarqube.credentialId', select: 'name' },
          { path: 'kubernetes.credentialId', select: 'name' }
        ]
      })
      .sort({ triggerDate: -1 })
      .limit(5);

    // Transform the data to include pipeline details in the activity object
    const transformedActivities = activities.map(activity => {
      const activityObj = activity.toObject();
      const pipeline = activityObj.pipelineId;

      if (pipeline) {
        // Add pipeline details to the activity object
        activityObj.type = pipeline.type;
        activityObj.github = pipeline.github;
        activityObj.docker = pipeline.docker;
        activityObj.aws = pipeline.aws;
        activityObj.sonarqube = pipeline.sonarqube;
        activityObj.kubernetes = pipeline.kubernetes;
        activityObj.testingTools = pipeline.testingTools;
        activityObj.continuousDeployment = pipeline.continuousDeployment;

        // Convert testingTools object to array for frontend compatibility
        if (pipeline.testingTools) {
          const testingToolsArray = [];
          if (pipeline.testingTools.unitTesting) testingToolsArray.push('Unit Testing');
          if (pipeline.testingTools.integrationTesting) testingToolsArray.push('Integration Testing');
          if (pipeline.testingTools.e2eTesting) testingToolsArray.push('E2E Testing');
          activityObj.testingToolsArray = testingToolsArray;
        }

        // Add security tools (SonarQube)
        activityObj.security = {
          tools: pipeline.sonarqube?.credentialId ? ['SonarQube'] : []
        };

        // Add deployment information
        activityObj.deployment = {
          platform: pipeline.continuousDeployment?.platform || null,
          namespace: pipeline.kubernetes?.namespace || null
        };
      }

      // Remove the populated pipelineId to avoid confusion
      delete activityObj.pipelineId;
